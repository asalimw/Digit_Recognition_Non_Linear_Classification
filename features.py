...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    # Note that pcs and feature_means are learned from the training data set,
    # which should not be computed in this function using  X .

    centred_x = X - feature_means #first center data using the feature_means
    project_x = centred_x @ pcs
    X_project = project_x[:, :n_components]
    return X_project
