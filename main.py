...
def run_softmax_on_MNIST_mod3(temp_parameter=1):
    ...
    # YOUR CODE HERE
    train_x, train_y, test_x, test_y = get_MNIST_data()
    train_y_mod3, test_y_mod3 = update_y(train_y, test_y) #Label change using mod3 function in softmax

    theta_mod3, cost_function_history_mod3 = softmax_regression(train_x, train_y_mod3, temp_parameter, alpha=0.3,
                                                                lambda_factor=1.0e-4,
                                                                k=10, num_iterations=150)

    plot_cost_function_over_time(cost_function_history_mod3)
    test_error = compute_test_error_mod3(test_x, test_y_mod3, theta_mod3, temp_parameter)
    # Save the model parameters theta obtained from calling softmax_regression to disk.
    write_pickle_data(theta_mod3, "./theta.pkl.gz")
...
print('softmax test_error (PCA_Test):', run_softmax_on_MNIST_PCA(temp_parameter=1))
...
print('softmax test_error (CUBE_Test):', run_softmax_on_MNIST_CUBE(temp_parameter=1))
